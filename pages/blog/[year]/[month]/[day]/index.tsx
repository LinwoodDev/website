...
export default function BlogDayPage({ allPosts }: { allPosts: PostType[] }) {
    ...
    return (
        <>
            ...
            <main>
                <Container size="sm">
                    <Title order={1}>Blog {year}-{('0' + month).slice(-2)}-{('0' + day).slice(-2)}</Title>
                    <Space h={"xl"} />
...
export const getStaticProps = async ({ params }: Params) => {
    ...
    const allPosts = getAllPosts()
        .filter((post) => post.date.year === parseInt(year) && post.date.month === parseInt(month) && post.date.day === parseInt(day));
